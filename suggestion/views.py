...
from .forms import SuggestionForm, ReplyForm
  ...
  def dispatch(self, request, *args, **kwargs):
    ...
    if is_user(self.request.user):
      if self.request.user != obj.suggestor:
        messages.error(request, "you are not authorized to reject this suggestion")
...
class AddReplyView(CreateView):
  ...
  def form_valid(self, form):
    ...
    form.instance.suggestion = Suggestion.objects.get(pk=self.kwargs.get('pk'))
